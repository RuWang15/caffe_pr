...
template <typename Dtype>
void caffe_gpu_gemm_ipdf(const int N, const int M, const int K, 
    const Dtype* A, const Dtype* B, Dtype* C, const bool trans, 
    const int T, const int start);

template <typename Dtype>
void caffe_gpu_gemm_ipdb_w(const int N, const int M, const int K, 
    const Dtype* A, const Dtype* B, Dtype* C, const bool trans, 
    const int T, const int start);

template <typename Dtype>
void caffe_gpu_gemm_ipdb_bd(const int N, const int M, const int K, 
    const Dtype* A, const Dtype* B, Dtype* C, const bool trans, 
    const int T, const int start);
